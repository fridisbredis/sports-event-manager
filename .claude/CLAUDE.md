...
**Sentry (4 per environment, `DEV_`/`PROD_`-prefixed like everything else):** `*_SENTRY_DSN`, `*_SENTRY_ORG`, `*_SENTRY_PROJECT`, `*_SENTRY_AUTH_TOKEN`. Two separate Sentry projects (`viadal-event-dev`, `viadal-event-prod`), so the DSN differs per environment — an earlier version of this section claimed one shared project and unprefixed names, which was wrong. `SENTRY_ENVIRONMENT` (`development`/`production`) is hardcoded per workflow, not a secret.

`*_SENTRY_AUTH_TOKEN` reaches the app through two independent channels with different guarantees — as of SYS-03 (2026-09-02), it is no longer build-only. At build time it's a Docker `ARG` only, never a Dockerfile `ENV`, so it is never persisted into the pushed image's filesystem layers. Separately, both deploy workflows also pass it to `az containerapp update --set-env-vars`, the same mechanism `SUPABASE_SERVICE_ROLE_KEY` already uses — Container Apps injects it straight into the running process's environment, with no image involved. The token has `project:read`/`org:read` scope only (confirmed 2026-09-02), which is what makes exposing it to the runtime process acceptable; SYS-03's `/admin/health` reads it there to query Sentry's API for live issue counts. If this scope is ever widened, re-examine whether runtime exposure is still fine.
