...
> v0.3 changes (2026-07-01, from a working session with Frida): operating windows can now be marked **recurrent daily** (a per-window checkbox) so a window repeats each day across a multi-day stage instead of being re-entered; added the cross-cutting **unsaved-changes guard** reference. See "Operating windows" below.
...
- **Recurrent daily windows (DECISION 2026-07-01):** each window is a **one-time** window by default. When a window is shorter than 24 hours **and its stage spans more than 24 hours**, the admin can tick a per-window **"Recurrent daily"** checkbox; the window then repeats each day for the **duration of the stage** rather than being entered once per day. This is what makes the Viadal 6-day case practical: "Toa check at 07:00 / 12:00 / 18:00" is entered once and recurs daily across the stage, instead of 18 separate windows. The recurrence is **gated on the stage's duration** (not the event's), because operating windows belong to a stage and the recurrence spans that stage; a short setup/teardown stage inside a long event therefore does not offer the option. Each recurring instance must still sit within the stage's allocable range; instances that would fall outside it are not generated.
...
- **Unsaved-changes guard (DECISION 2026-07-01):** WS-02 saves via an explicit **Save** button, and edits made in its modals are lost if the admin navigates away before saving. Navigating away with unsaved changes triggers the shared confirmation dialog (**Save** / **Discard and continue** / **Cancel**). Shared cross-cutting pattern, specified once in `docs/screens/screen-documentation.md`.
