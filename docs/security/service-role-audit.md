...
- **Unauthenticated write** — the write must succeed on paths that prove
  nothing about the caller (e.g. a failed OTP verify has no `auth.uid()` at
  all), so there is no session to key an RLS INSERT policy off of.
...
| 22  | `src/lib/rate-limit.ts`                           | `checkLoginSendRateLimit`/`checkLoginVerifyRateLimit` (via `service.rpc('check_rate_limit')`)                                 | Postgres-based rate limiting for the OTP login send/verify endpoints (SEC-08)     | None — this check runs before any session exists (login itself)                                                                                             | No RLS equivalent — same `check_rate_limit` RPC as row #21, `service_role`-only                                                                               | `src/lib/rate-limit.test.ts` (unit) + `tests/integration/rate-limit.test.ts` (integration, real RPCs)                                                                   |
| 23  | `src/lib/audit/log-auth-event.ts`                 | `insert` into `auth_events` (SEC-07-rest)                                                                                     | Records OTP send/verify success and failure as audit events                      | None by construction — a failed OTP verify has no authenticated caller; a successful one is recorded before the route reads back the resulting session      | Unauthenticated write — no `auth.uid()` exists to satisfy any RLS INSERT policy, not even a token-gated one (see docs/adr/0001 category 5)                    | `src/lib/audit/log-auth-event.test.ts` (unit) + `src/app/api/auth/send-otp/route.test.ts`, `src/app/api/auth/verify-otp/route.test.ts`                                  |
