...
| Lint | Failed: the `next lint` script is incompatible with the installed Next.js version |
...
| F-MNT-01 | `npm run lint` fails because it invokes `next lint`, which is not supported by the installed Next.js release. | A key quality gate is currently ineffective. | Configure ESLint's supported CLI and add it to pull-request CI. | High |
