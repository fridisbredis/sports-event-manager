...
- **Last verified:** 2026-08-11 after the lint script was updated
...
| Lint                 | The script now correctly invokes `eslint .`, but the check reports 4 errors and 14 warnings that must be resolved or explicitly accepted by policy.                                           |
...
| CI/SAST              | CodeQL is configured for pushes, pull requests, manual runs, and weekly scans                                                                                                                 |
...
| F-MNT-01  | The lint script has been corrected to use `eslint .`. The now-active check reports 4 errors (synchronous state updates inside effects) and 14 warnings, including unused variables and a missing Hook dependency. | The quality gate now detects real maintainability and potential rendering-performance problems, but it cannot pass until the findings are resolved or deliberately configured. | Fix the four errors; remove or justify the warnings; agree whether warnings fail CI; then add the passing lint check to pull-request CI.    | High     |
