...
**Version:** v0.7 · **Date:** 2026-06-29 · **Owner:** Peter Thorn (product lead) · **Dev:** Frida Bredberg

**v0.7 changes (Stage model):** clarified and expanded the existing **Stage** concept (previously "Stage/Day"); structure is now Event → Stage → Work area. A stage now has a type, **Race** or **Non-race**, its own start/end time, and an optional venue. "Workstation" is renamed **Work area** throughout and now belongs to exactly one stage. Distances move onto Race stages (event-level distance is a read-only roll-up). Mandatory-to-publish is now "at least one Race stage". Screen/file IDs (WS-01/WS-02, SCHED-01) are kept stable; only labels change.

## Problem

Organizers of multi-day endurance events plan and run their events today on spreadsheets, paper, and scattered messaging. Officials, schedules, work-area duties, participant details, and results live in disconnected places. This causes version confusion, manual handoffs, and fragile execution on event day. There is no single, reliable system to configure an event, invite and schedule officials against work areas and timeslots, give participants their own information, and communicate reliably with both groups. Viadal is the design driver, but the same coordination problem exists across small sport events run by small organizing teams.
...
- **Official (Event funktionär):** invited via SMS. Assigned to work areas, to-do lists, and timeslots. Views own schedule.
...
1. **Event creation and configuration:** event type, dates, **stages** (each Race or Non-race, with its own start/end time and an optional venue; Race stages also carry distance(s) in time or distance), location, facilities, logo, description. Event-level distance is a read-only roll-up of the Race stages' distances.
2. **Officials:** registration via SMS invite, management, and assignment scheduling (work areas, to-do lists, timeslots), **scoped per stage**. Scheduling is presented in two complementary, both-editable views over a single assignment model: a **per-person view** (people as columns, timeslots as rows) to read how loaded each official is, and a **per-work-area view** (work areas/to-do as columns, timeslots as rows) to read whether each post is staffed against its capacity ceiling. Editing in one view updates the other. The scheduling granularity (the configurable timeslot length) is set per event (1 hour is the Viadal default).
...
5. **Communication:** two separate announcement channels (participants and officials), admin-publish only, with SMS notifications on by default and opt-out in personal account settings. (Planned for a later version, not v1: per-work-area channels and two-way/interactive officials messaging with an admin "important" checkbox that triggers SMS.)
...
- **Event** → **Stage** → **Work area**: an event is a sequence of stages; each stage groups the work areas staffed during it. Event type is a configured attribute.
- **Stage:** a phase or segment of the event with its own start and end time and an optional **venue** (course, route, pool, etc.). Each stage has a **type**: **Race** (participant-facing competition segment; carries one or more distances/lengths in time or distance, plus a formal start and end time) or **Non-race** (officials/admin-only operational work such as setup, teardown, reporting, social media; not shown to participants). An event has one or more stages and must have **at least one Race stage** to publish. A multi-segment competition (e.g. a stage race) is modelled as **multiple Race stages**; Viadal 6-day is a **single Race stage spanning six days**. The UI pre-fills three template stages (setup, race, teardown), all editable and removable down to the one-Race-stage minimum.
- **Work area:** (formerly "workstation") a unit of work officials are assigned to, physical or not (timing point, aid station, finish line, but also social media, purchasing). **Belongs to exactly one stage.** Carries an "up to X" capacity ceiling (a maximum, not a per-timeslot minimum) and one or more operating windows. Operating windows must fall within the stage's **allocable range**: for a **Race** stage that range is the formal start/end **extended by up to one hour before and one hour after** (covering pre-start assembly and post-finish measurement); for a **Non-race** stage it is the stage's start/end. The per-work-area view flags over-capacity; staffing below the ceiling is normal and not flagged. Timeslots outside a work area's windows are not allocable.
