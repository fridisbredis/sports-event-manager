...
- [x] Fix implementerad 2026-08-18: `toTwilioE164` (`src/lib/phone.ts:32`) lägger tillbaka `+` vid de tre — och enda — Twilio-anropen: `officials/route.ts:105`, `officials/[id]/resend/route.ts:87`, `announcements/route.ts:70`. Tolererar rader som redan har `+` så de äldre raderna inte blir `++`. `formatPhoneForDisplay` återanvänder samma hjälpare. `signInWithOtp`-anropen är medvetet **orörda** — Supabase Auth normaliserar själv och de fungerar redan. Enhetstester: 28 filer / 267 tester passerar, `tsc --noEmit` rent. **Deployad till dev 2026-08-18** — mergad till `main` i `859b0be`, och `deploy-dev.yml` auto-deployar varje push till `main` (image-tagg `dev-<sha>`). Workflow-körningens utfall är dock **inte bekräftat**: varken `gh` eller `az` är installerat lokalt och appen exponerar ingen build-SHA, så koden kan inte fingeravtryckas mot den körande containern — kolla Actions → "Deploy to dev" om det behöver beläggas. **Fortfarande ej manuellt verifierad** — de tre punkterna nedan kräver ett riktigt utskick och är oberörda av deployen.
...
- [ ] **Befintliga rader lagrade med `+` — normalisering, inte bara visning.** Punkten ovan om konsekvent formatering täcker bara _visningen_. De lagrade värdena är fortfarande blandade, och rader med `+` kan inte bekräftas via 0017/0018 överhuvudtaget (exakt strängmatchning mot `auth.users.phone` som saknar `+`). Kartlägg med SQL-frågan i avsnitt 6 och besluta om `update officials set phone = ltrim(phone, '+')` ska köras i båda miljöerna. Måste ske **före** 0020 appliceras på prod.
- [ ] **0020:s index fångar inte `+`/utan-`+`-dubbletter.** Indexet är på `(tenant_id, phone)` som strängar, så samma person kan ha två aktiva rader med olika format. Verifiera efter en eventuell normalisering att dubblettkollen fortfarande ger noll rader — normaliseringen kan _skapa_ dubbletter som indexet sedan vägrar.
