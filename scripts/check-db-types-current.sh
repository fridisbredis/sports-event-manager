...
#   }
#
# That value is infrastructure, not schema. It has flipped between '14.5' and
# '14.17' on the dev project alone — '14.5' originally, '14.17' from the
# PERF-04 commit, '14.5' again as of 2026-08-31 — with no migration involved.
...
# Everything except the PostgrestVersion value. Anchored to the field name so a
# version string appearing anywhere else in the file is still compared.
strip_version() {
  sed -E "s/(PostgrestVersion: ')[^']*(')/\1IGNORED\2/"
}
